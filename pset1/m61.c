...
void* m61_malloc(size_t sz, const char* file, int line) {
	...
	if (sz < SZ_MAX) {
		...
		metadata_ptr = (struct statsNode*) malloc(sizeof(struct statsNode) + sz + sizeof(struct footNode));
...
void m61_free(void *ptr, const char *file, int line) {
	...
	if ((char*) ptr > (char*)(user_stats.heap_max + sizeof(struct footNode)) || (char*) ptr < (char*)(user_stats.heap_min + sizeof(struct statsNode))) {
		printf("MEMORY BUG: invalid free of pointer %p, not in heap\n", ptr);
		exit(0);
	} else {
		/* Setup pointers to the Metadata Struct & the Footer */
		...
		/* Validate against pointer inaccuracies */
		if (metadata_ptr->ptr != ptr) {
			...
			exit(0);
		} else if (metadata_ptr->active == 0 || metadata_ptr->head != 88 ) {
...
void m61_hhtestreport(void) {
	 struct statsNode* traverse = root;
	 while (traverse != NULL) {
		size_t sz = traverse->sz;
		if (sz > ((unsigned long long) 0.2 * user_stats.total_size)) {
			const char* file = traverse->file;
			int line = traverse->line;
			printf("HEAVY HITTER: %s:%i: %zu bytes ()\n", file, line, sz);
		}
		traverse = traverse->prev;
	}
}
